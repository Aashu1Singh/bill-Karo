    ...
    return (
        ...
        <form onSubmit={handleSubmit} className='row my-3 mb-2' style={{ color: props.mode === 'dark' ? 'white' : 'black' }} >
            ...
            <div className="row mx-1">
                <button type="submit" disabled={!(name.length >0 )} className="btn btn-secondary btn-sm col-md-3 " style={{ color: props.mode === 'dark' ? 'white' : 'black' }} onClick={() => { return props.addItem(name, Number(price)) }}><strong > Add Item</strong></button>
            </div>
        </form>
    )
}
